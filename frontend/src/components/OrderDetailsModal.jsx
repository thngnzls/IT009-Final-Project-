...
import { FaStar, FaShippingFast, FaReceipt, FaMapMarkerAlt } from 'react-icons/fa';
...
const ShipmentStatus = ({ orderStatus }) => {
    ...
    if (activeIndex === -1 && orderStatus === 'Shipped') activeIndex = statusKeys.indexOf('In Transit');
    
    // Handle non-trackable statuses
    const nonTrackableStatuses = ['Cancelled', 'Return/Refund Requested', 'Return Approved', 'Return Rejected', 'Payment Failed and Order Cancelled'];
        ...
        return (
            <div className="mt-4 p-4 border rounded-lg bg-red-50 shadow-sm">
                <h3 className="text-lg font-semibold flex items-center mb-3 text-gray-800">
                    <FaShippingFast className="mr-2 text-red-600" /> Order Status: <span className='ml-2 font-bold text-red-600'>{orderStatus}</span>
                </h3>
                <p className='text-sm text-gray-600'>This order is no longer in the standard shipment process.</p>
            </div>
        );
    }
    ...
    return (
        <div className="mt-4 p-4 border rounded-lg bg-white shadow-md">
            <h3 className="text-lg font-semibold flex items-center mb-10 text-gray-800">
                <FaShippingFast className="mr-2 text-indigo-600" /> Current Status: <span className='ml-2 font-bold text-indigo-600'>{orderStatus}</span>
            </h3>
            ...
            <div className="relative pt-4">
                ...
                <div className="absolute top-5 left-0 h-1 bg-indigo-600 rounded transition-all duration-700 ease-in-out" 
                ...
                <div className="flex justify-between relative">
                    {statuses.map((status, index) => {
                        ...
                        return (
                            ...
                            <div key={status.key} className="flex flex-col items-center w-[20%]"> 
                                {/* Dot (z-10 to appear over the line) */}
                                <div className={`w-5 h-5 rounded-full border-4 transition-all duration-300 z-10 mb-2 ${
                                    isCompleted 
                                ...
                                <div className="mt-2 text-xs text-center leading-snug w-full">
                                    <span className={isCompleted ? 'text-gray-800 font-semibold' : 'text-gray-500'}>
                                        {status.label}
                                    </span>
                                </div>
                            </div>
...
const TrackingMapPlaceholder = ({ orderStatus }) => {
    ...
    const isTrackable = !['Delivered', 'Cancelled', 'Return/Refund Requested', 'Return Approved', 'Return Rejected', 'Payment Failed and Order Cancelled'].includes(orderStatus);
    ...
    const staticMapUrl = "https://images.unsplash.com/photo-1596482159800-47b7194f1f54?q=80&w=2000&auto=format&fit=crop&ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D"; 
    ...
    return (
        <div className="mt-6 p-4 border rounded-lg bg-white shadow-inner">
            <h3 className="text-lg font-semibold flex items-center mb-3 border-b pb-2 text-gray-800">
                <FaMapMarkerAlt className="mr-2 text-red-500" /> Live Tracking (Simulated)
            </h3>
            <div className="relative h-64 w-full rounded-md overflow-hidden border border-gray-200">
                ...
                <div className={`absolute ${pinPosition} transform -translate-x-1/2 -translate-y-1/2 z-10`}>
                    <FaMapMarkerAlt 
...
const OrderDetailsModal = ({ order, productToRate, onClose, onReviewSubmit }) => {
    ...
    const handleSubmit = (e) => {
        ...
        setTimeout(() => {
            ...
            setIsSubmitting(false);
            setMessage(productToRate ? 'Review submitted successfully! (Simulated)' : 'Closed.');
    ...
    return (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white p-6 sm:p-8 rounded-lg shadow-xl w-11/12 max-w-2xl max-h-[90vh] overflow-y-auto relative">
                ...
                <div className='mb-4'>
                    <Title text1={`ORDER`} text2={`#${order._id ? order._id.substring(0, 8) : 'DETAILS'}`} />
                </div>
                ...
                <div className="mt-6 p-4 border rounded-lg">
                    ...
                    <div className="space-y-2 mb-4 max-h-40 overflow-y-auto">
                        {order.items && order.items.map((item, index) => (
                            <div key={index} className="flex justify-between text-sm text-gray-600">
                                ...
                                <span className='text-right'>Qty: {item.quantity} × {formatCurrency(item.price)}</span>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="border-t pt-2 text-gray-800">
                        <div className="flex justify-between font-medium">
                            ...
                            <span className='font-semibold'>{order.paymentMethod ? order.paymentMethod.toUpperCase() : 'N/A'}</span>
                        </div>
                        <div className="flex justify-between font-medium">
                            <span>Order Date:</span>
                            <span className='font-semibold'>{new Date(order.date).toDateString()}</span>
                        </div>
                ...
                {productToRate && (
                    <div className="mt-6 p-4 border rounded-lg bg-yellow-50">
                        <h3 className="text-lg font-bold mb-3 text-gray-800">
                            <FaStar className='inline mr-2 text-pink-500' /> Rate Product: {productToRate.name}
                        </h3>
                        ...
                        <form onSubmit={handleSubmit} className="mt-4">
                            ...
                            <div className="text-center mt-4">
                                {message && <p className={`text-sm my-2 ${message.includes('success') ? 'text-green-600' : (message.includes('Please select') ? 'text-red-600' : 'text-gray-600')}`}>{message}</p>}
                                <button
                                    type="submit"
